...
class Password
{
	...
	boolean hasUppercaseCharacterOrSpecialCharacter()
	{
		...
		for (int i = 0; i < password.length(); i++)
		{
			...
			else if (!(password.charAt(i) >= 'a' && password.charAt(i) <= 'z') && !(password.charAt(i) >= '0' && password.charAt(i) <= '9' ))
			{
	...
	boolean hasImmediatePattern()
	{
		for (int i = 0; i < password.length(); i++)
		{
			for (int j = i + 1; j < password.length(); j++)
			{
				...
				/*
				int k;
				for (k = 0; k < pattern.length() && j + k < password.length(); k++)
				{
					if (password.charAt(j + k) != pattern.charAt(k))
					{
						break;
					}
				}
...
public class CheckPassword 
{
	public static void main(String[] args)
	{
		...
		if (checkPassword.hasLowercaseCharacterAndDigit() && !checkPassword.hasUppercaseCharacterOrSpecialCharacter() && checkPassword.hasRequiredLength() && !checkPassword.hasImmediatePattern())
		{

...
abstract class QuizResult {
	protected char[] correctAnswers = { 'C', 'A', 'B', 'D' };
	protected char[] submittedAnswers = new char[4];
	protected Status[] result = new Status[4];
	public final int PASS_MARKS = 2;
...
class Report extends QuizResult {
	...
	void printReport() {
		...
		System.out.println("---------------------------------------------------------------");
		for (int i = 0; i < 4; i++) {
			System.out.printf("| %8d | %17c | %15c | %10s |\n", (i + 1), submittedAnswers[i], correctAnswers[i],
					result[i]);
			System.out.println("---------------------------------------------------------------");
		}
